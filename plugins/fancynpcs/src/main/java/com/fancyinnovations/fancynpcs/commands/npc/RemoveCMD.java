...
import com.fancyinnovations.fancynpcs.api.events.NpcStopLookingEvent;
...
public enum RemoveCMD {
    ...
    public void onRemove(
        ...
        // Calling both events for backwards compatibility (NpcRemoveEvent is deprecated)
        NpcDeleteEvent deleteEvent = new NpcDeleteEvent(npc, sender);
        @SuppressWarnings("deprecation")
        NpcRemoveEvent removeEvent = new NpcRemoveEvent(npc, sender);

        if (deleteEvent.callEvent() && removeEvent.callEvent()) {
            npc.removeForAll();
